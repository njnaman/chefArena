...
$app->get('/contest/{contestname}/{problem}', function (Request $request, Response $response, $args) {

    $contest_name = $request->getAttribute('contestname');
    $problem = $request->getAttribute('problem');
    try{
      $db = new database();
      $db = $db->connect();
      ...
      $Rresponse = $_SESSION['guzzle']->request('GET','/contests/'.$contest_name.'/problems/'.$problem,['headers'=>array('Content-Type'=>'application/json','Authorization'=>'Bearer '.$access_token)]);
      if($Rresponse->getStatusCode()!=200)
          {throw new Exception("failed");}
      $body = $Rresponse->getBody();
      $res = json_decode($body,true);
