...
$app->get('/contests', function (Request $request, Response $response, $args) {
    ...
    try{
        ...
        //contest list
        $Rresponse = $_SESSION['guzzle']->request('GET','/contests',['headers'=>array('Content-Type'=>'application/json','Authorization'=>'Bearer '.$access_token)]);
        if($Rresponse->getStatusCode()!=200)
            {throw new Exception("failed");}
        $body = $Rresponse->getBody();
        $res = json_decode($body,true);
